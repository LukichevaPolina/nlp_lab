...
class TextClassificationPipeline:
    def __init__(
        ...
        self._checkpoint_path = self.get_checkpoint(
            algorithm, embeddings, class_balancer, preprocessor)
        self._metrics_path = self.get_metrics_name(
            algorithm, embeddings, class_balancer, preprocessor)
    ...
    def train(self, X_train, y_train, X_test, y_test) -> None:
        match self._algorithm:
            ...
            case Algorithm.CNN:
                train_metrics, val_metrics, train_losses, val_losses = cnn_train(
                    self._checkpoint_path, X_train, y_train, X_test, y_test, num_epochs=100, batch_size=64)
            ...
            case Algorithm.LINEAR:
                train_metrics, val_metrics, train_losses, val_losses = linear_train(
                    self._checkpoint_path, X_train, y_train, X_test, y_test, num_epochs=100, batch_size=64)
                pd.DataFrame(train_metrics).to_csv(
                    f"metrics/train_metrics_{self._metrics_path}.csv")
                pd.DataFrame(val_metrics).to_csv(
                ...
                pd.DataFrame(val_losses).to_csv(
                    f"metrics/val_losses_{self._metrics_path}.csv")
                plot_learning_curve(
                    train_losses["ce"], val_losses["ce"], name="linear_learning_curve")
                plot_accuracy_curve(
                    train_metrics["accuracy"], val_metrics["accuracy"], name="linear_accuracy_curve")
                plot_f1_curve(
                    train_metrics["f1score"], val_metrics["f1score"], name="linear_f1_curve")
            case _:
                raise ValueError(f"Given algorithm: {
                                 self._algorithm} does not exist")
    ...
    def eval(self, X, y) -> None:
        ...
        if self._algorithm == Algorithm.SVM or self._algorithm == Algorithm.DECISION_TREE:
            ...
            eval_metrics = {"f1_per_class": f1_per_class,
            ...
            f1_per_classes = f1_score(
                y, y_pred, average=None)  # TODO add computation
            f1_per_class = dict((i, j) for i, j in enumerate(f1_per_classes))
            accuracy = balanced_accuracy_score(y, y_pred)
            f1 = f1_score(y, y_pred, average="weighted")
            eval_metrics = {"f1_per_class": f1_per_class,
            ...
            raise ValueError(f"Imposible eval, not algorithm = {
                             self._algorithm}")

    def infer(self, X) -> None:
        if not self._checkpoint_path.exists():
            raise RuntimeError(f"The checkpoint: {str(
                self._checkpoint_path)} does not exist, train first")
        ...
        if self._algorithm == Algorithm.SVM or self._algorithm == Algorithm.DECISION_TREE:
            ...
            y = linear_infer(self._checkpoint_path, X)
            return y
        else:
            NotImplementedError

    def save_results(self, name, metrics) -> None:
        file = "metrics/" + name + f"_{self._metrics_path}.json"
    ...
    def get_checkpoint_name(
        ...
        if algorithm in ["svm", "decision-tree"]:
    ...
    def get_checkpoint(
        ...
        checkpoint_name = Path(self.get_checkpoint_name(
            algorithm, embeddings, class_balancer, preprocessor))
    ...
    def get_metrics_name(
        ...
        if algorithm in ["linear", "cnn", "svm", "decision-tree"]:
            return f"{algorithm}_{embeddigns}_{class_balancer}_{preprocessor}"
        else:
            raise ValueError(f"Given algorithm: {algorithm} does not exist")
